...
class AIDirector:
    ...
    def generate_llm_prompt(self, mechanical_result: str, context: str, intent_raw: str = None, filters: str = "") -> str:
        ...
        full_prompt = (
            ...
            f"4. DO NOT just describe empty scenery. YOU ARE THE AGGRESSOR. Force the player to react.\n"
            f"{action_directive}\n"
            f"5. IMPORTANT: End your response with a direct question, an immediate physical threat, or a ticking clock consequence so the player MUST react.\n"
            f"6. Output ONLY raw JSON matching the required schema.\n"
            f"<|end|>\n"
            f"<|assistant|>\n"
            f"{{\n"
        )
    ...
    def build_director_prompt_with_spine(self, location_name: str, local_lore: str, subtle_seeds: list, campaign_weaver, filters: str = "", intent_raw: str = None, mechanical_result: str = "", scene_script: str = "") -> str:
        ...
        full_prompt = (
            ...
            f"8. AGENCY HANDOFF: End your narration with an immediate consequence, threat, or by asking a direct question to demand a reaction from the player.\n"
            f"{action_directive}\n"
