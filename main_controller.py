...
from world_manager.scene_initializer import SceneInitializer
from world_manager.objective_system import ObjectiveSystem
from world_manager.dialogue_system import DialogueSystem
...
class VoiceSignalBridge(QObject):
    voice_update = pyqtSignal(dict)

from core.event_bus import event_bus
from core.async_workers import AsyncWorkerPool
...
class SagaController:
    def __init__(self):
        ...
        self.worker_pool = AsyncWorkerPool(ai_director=self.ai)
        ...
        self.npc_ai = NPCAI(clash_calculator=self.rules)
        self.lore = SimpleLore()
        self.map_engine = ClusterManager()
        
        self.world_db = WorldDB("okasha_world.db")
        self.scene_init = SceneInitializer(self.world_db, self.story, self.ai)
        self.objective_system = ObjectiveSystem(ecs_manager=None, scene_initializer=self.scene_init, map_generator=self.map_engine, story_framework=self.story)
        self.dialogue_system = DialogueSystem(ecs_manager=None, clash_calculator=self.rules, ai_director=self.ai)
        ...
        self.voice_bridge.voice_update.connect(self._on_voice_update_main_thread)
        
    def _on_voice_update_main_thread(self, tags: dict):
        intent = tags.get("dm_data", {}).get("last_intent", "")
        narration = tags.get("narration_text", "")
        if intent:
            self.bus.publish("NARRATIVE_OUTPUT", {"response": f"[VOICE INTENT: {intent}]\n{narration}"})
        else:
            self.bus.publish("NARRATIVE_OUTPUT", {"response": narration})
            
        # Forward dm_data to the UI for Story Tracker and DM Dashboard updates
        if "dm_data" in tags:
            self.bus.publish("HUD_UPDATE", {"dm_data": tags["dm_data"]})
        
    def _start_voice_engine(self):
        def voice_callback(tags):
            self.voice_bridge.voice_update.emit(tags)
        
        engine = VoiceEngine(
            ui_callback=voice_callback, 
            rules_callback=self.rules.resolve_action, 
            player=self.player,
            ai_director=self.ai
        )
        engine.run_loop()
    ...
    def _init_world(self, payload=None):
        ...
        # Phase 1: DM's Notebook (Pre-gen the Scene)
        cell_id = "cell_0_tutorial" if self.story.current_act == 0 else "cell_0001"
        if payload and "cell_id" in payload:
            cell_id = payload["cell_id"]
            
        if self.player:
            self.player.current_cell_id = cell_id
            
        blueprint = self.scene_init.prep_scene(cell_id)
        
        poi_coords = [[14, 22]]
        self.map_engine.generate_cluster(region_id=1, base_biome="Forest", poi_coords=poi_coords, poi_context="Starting Zone", blueprint=blueprint)
        self._update_map_render()
        self._update_hud()
        
        if not self.voice_thread or not self.voice_thread.is_alive():
            self.voice_thread = threading.Thread(target=self._start_voice_engine, daemon=True)
    ...
    def _update_hud(self):
        if self.player:
            ...
            self.bus.publish("HUD_UPDATE", {"character": char_data})
        else:
    ...
    def _handle_buy_item(self, payload):
        ...
        self.bus.subscribe("PLAYER_MOVE_VTT", self._handle_vtt_move)
        self.bus.subscribe("SPAWN_VTT_ENTITIES", self._handle_vtt_spawn)
        
    def _handle_vtt_move(self, payload):
        direction = payload.get("direction", "").lower()
        if not self.current_battlemap: return
        import random
        # Small visual translation on the map to reflect movement
        dx, dy = 0, 0
        if "melee" in direction or "forward" in direction or "engage" in direction:
            dx = random.randint(-1, 1)
            dy = random.randint(-2, -1)
        elif "back" in direction or "away" in direction or "far" in direction:
            dx = random.randint(-1, 1)
        ...
        self.player_px = max(0, min(self.current_battlemap["width"]-1, self.player_px + dx))
        self.player_py = max(0, min(self.current_battlemap["height"]-1, self.player_py + dy))
        self._update_map_render()
        
    def _handle_vtt_spawn(self, payload):
        ...
        for e in entities:
            ...
            etype = e.get("type", "Prop")
            # Spawn them roughly around the player
            ex = max(0, min(self.current_battlemap["width"]-1, self.player_px + random.randint(-5, 5)))
            ey = max(0, min(self.current_battlemap["height"]-1, self.player_py + random.randint(-5, 5)))
    ...
    def handle_player_action(self, payload):
        ...
        # Handle Dialogue
        if intent_raw.startswith("say to ") or intent_raw.startswith("speak to ") or intent_raw.startswith("talk to "):
            import re
            match = re.match(r"(?:say to|speak to|talk to)\s+([a-zA-Z0-9_]+)\s*(?:saying|:)?\s*(.*)", intent_raw, re.IGNORECASE)
            if match:
                npc_name = match.group(1).strip()
                speech = match.group(2).strip()
                
                # Check for "say to NPC: message" or "speak to NPC message"
                if not speech:
                    parts = intent_raw.split(npc_name, 1)
                    if len(parts) > 1:
                        speech = parts[1].strip(" :")
                        
                if speech:
                    plot_hook = self.story.active_hook if self.story else "Wandering."
                    response = self.dialogue_system.speak_to_npc(npc_name, speech, self.current_battlemap, plot_hook)
                    self.bus.publish("SYSTEM_LOG", f"[{npc_name}]: {response}")
                    
                    # Use TTS if configured
                    self.bus.publish("NARRATIVE_OUTPUT", {"response": response, "audio": True})
                    
                    self.world.advance_time(1) # Talking takes a tick
                    self._update_hud()
                    return
        ...
        if moved:
            ...
            player_state = {"tags": self.player.tags if hasattr(self.player, "tags") else []}
            self.npc_ai.process_turn(entities, player_state)
        ...
        # 5. Check if the scene is finished
        self.objective_system.evaluate_scene(self.player)
    ...
    def _handle_vendor_trade(self, vendor_name: str):
        ...
        prompt = f"The player wants to trade with a vendor named {vendor_name} in {biome}. Based on this lore: {lore}. Generate exactly 3 unique items for sale. Return ONLY valid JSON in this exact format, with no markdown formatting or backticks: [{{ \"name\": \"Item Name\", \"cost\": 50, \"desc\": \"What it does\" }}]"
    ...
    def open_dm_dashboard(self):
        if not self.dm_dashboard:
            self.dm_dashboard = DMDashboard(self.ui)
        self.dm_dashboard.show()

    def run(self):
        # We need a way to open the DM Dashboard from the UI or code.
        # Let's bind it to a bus event just in case, and also allow manual opening.
        self.bus.subscribe("OPEN_DM_DASHBOARD", lambda p: self.open_dm_dashboard())
        self.ui.show()
